...
def calculate_control_inputs(controllable_vars_accels, state, desired_yaw, desired_vel_yaw,
    ...
    q_control = link_accel + gain_constants.K_Q_V * (desired_link_velocities - numpy.array(state.joint_velocities).transpose()) + \
        gain_constants.K_Q_P * (desired_link_positions - numpy.array(state.joint_positions).transpose())

    current_velocity = numpy.array([state.vx, state.vy, state.vz]).transpose()
    current_position = numpy.array([state.x, state.y, state.z]).transpose()
